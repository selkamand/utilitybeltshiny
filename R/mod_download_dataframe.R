...
mod_download_dataframe_server <- function(id, data_to_write, filename_full, rownames=FALSE, colnames=TRUE){
  moduleServer(id,
    function(input, output, session){
      ...
      assertthat::assert_that(assertthat::is.flag(rownames))
      assertthat::assert_that(assertthat::is.flag(colnames))
      ...
      output$out_download_bttn <- downloadHandler(filename = filename_full, content = function(file) {
        assertthat::assert_that(is.data.frame(data_to_write()), msg = utilitybeltassertions::fmterror("mod_download_dataframe_server: data_to_write() must be a dataframe or a datatable, not a ", class(data_to_write())))
